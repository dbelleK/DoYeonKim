...
public class Data {
	...
	//ArrayList <RestaurantDto> restaurantDtos = new  <RestaurantDto>(); // ArrayList만 사용가능 바꿀 수 없음
    private ArrayList<RestaurantDto> restaurantDtos; //ArrayList 선언

    public Data() {
        makeList();
    }

    public List<RestaurantDto> makeList(){ //메서드
        restaurantDtos = new ArrayList<>(); //RestaurantDto형으로 선언한 ArrayList 생성 //이렇게 따로따로 하면 ArrayList를 하든 벡터를 하든 바꿔줄 수 있음

        //새로운 맛집 인스턴스 생성//add()메서드로 요소 값추가
